...
def draw_borders(pillow_image, bounding, color, image_size, caption='', confidence_score=0):
    ...
    font=ImageFont.truetype('/home/pi/.fonts/arial.ttf', 22)
    
    draw.text((
        bounding.normalized_vertices[0].x * width,
        bounding.normalized_vertices[0].y * height),
        font=font, text=caption, fill=color)
    
    draw.text((
        bounding.normalized_vertices[0].x * width,
        bounding.normalized_vertices[0].y * height+20),
        font=font, text='Confidence Score: {0:.2f}'.format(confidence_score), fill=color)
        
    return pillow_image
    
def draw_borders_plate(pillow_image, bounding, color, image_size, caption=''):
    width, height = image_size
    draw = ImageDraw.Draw(pillow_image)
    draw.polygon([
        bounding.normalized_vertices[0].x * width,
...
def crop_image(img, bounding, image_size):
    width, height = image_size
    img_area=(
        bounding.normalized_vertices[0].x * width,
        bounding.normalized_vertices[0].y * height,
